        ...
        >
          ...
          {isComposerOpen ? (
            <form className="mt-4 grid gap-4" onSubmit={handleSubmit}>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="grid gap-2">
                  <span className="ui-text text-sm font-semibold text-foreground">Bible Question PDF</span>
                  <div className="event-form-input rounded-[16px] border border-dashed border-border/80 bg-white px-4 py-4">
                    <div className="flex min-w-0 items-center gap-2 text-sm text-muted-foreground">
                      <Upload className="size-4" />
                      <span className="block min-w-0 truncate">{questionDocument?.name ?? existingQuestionDocName ?? "Upload a PDF file"}</span>
                    </div>
                    <input
